...
#define EVENT_DISABLE_IRQ() portDISABLE_INTERRUPTS()
#define EVENT_ENABLE_IRQ() portENABLE_INTERRUPTS()
...
#define EVENT_GET_TIMESTAMP() millis()
#endif
...
typedef enum {
  SUBS_MODE_NORMAL =
      0, /* 1,normal模式下，订阅者保存消息队列，只要给定的消息队列长度足够就不会漏消息，注册的时候直接把处理
                函数传参进去即可
            2,normal模式下，一个订阅者可以订阅任意多个event */

  SUBS_MODE_NOLIST, /* 1,noList模式下，订阅者中每一个eventID只申请一个内存单元，保留最新的消息，需要使用
                       EventMsgGetLast 拷贝数据
...
/* 消息回调 */
typedef void (*msgCallBack_f)(uint32_t eventID, void *pMsgData,
                              uint32_t timeStamp);
...
int EventSubscribeInit(struct Subscriber_t *pSubscriber, subscribeMode_t mode);
int EventSubscribe(subscriber_t *pSubscriber, uint32_t eventID,
                   uint32_t msgSize, uint32_t msgAddNum,
...
int EventMsgGetLast(subscriber_t *pSubscriber, uint32_t eventID, void *pMsgAddr,
                    uint32_t *pTimeStamp);
#ifdef __cplusplus
}
#endif
